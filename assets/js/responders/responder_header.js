(function () {
  ...
  function openResponderProfile() {
    window.location.href = '/HANDAVis/roles/responders/responders-profile.php';
  }

  function updateSearchPlaceholder() {
    var input = document.getElementById('dashboardSearch');
    if (!input) return;
    input.placeholder = window.innerWidth <= 940 ? 'Search' : 'Search assignments, map, or resources...';
  }

  window.toggleTheme = toggleTheme;
  window.handleSearch = handleSearch;
  window.toggleHamburger = toggleHamburger;
  window.openResponderProfile = openResponderProfile;
